...
class MariachiAppPremium {
    ...
    setupScrollEffects() {
        // El listener de scroll del nav es manejado por lenis-init.js cuando Lenis está disponible.
        // Este fallback garantiza compatibilidad si Lenis no carga (e.g., sin red).
        if (!window.lenis) {
            const nav = document.getElementById('mainNav');
            window.addEventListener('scroll', () => {
                const currentScroll = window.pageYOffset;
                if (nav) nav.classList.toggle('scrolled', currentScroll > 50);
            });
        }

        // Premium Scroll Revealing Animations
        ...
        this.observeElements = () => {
            ...
            }, 300); // Wait for DOM to render content
        };
        
        this.observeElements();
        window.observeElements = this.observeElements;
    }
    ...
    loadSection(sectionName) {
        ...
        // Scroll to top — usa Lenis si está disponible, fallback nativo si no
        if (window.lenis) {
        ...
        // Notificar a GSAP que la sección está lista para animar.
        // El delay de 400ms da tiempo a que sections-premium.js inyecte el HTML.
        setTimeout(() => {
            window.dispatchEvent(new CustomEvent('mariachi:sectionReady', { detail: { section: sectionName } }));
        }, 400);
    }
    ...
    loadHomeSection() {
        ...
        // Notificar a GSAP que el home está listo para animar
        setTimeout(() => {
            window.dispatchEvent(new CustomEvent('mariachi:sectionReady', { detail: { section: 'home' } }));
        }, 400);
    }
    ...
    loadAnunciosSection() {
        const content = document.getElementById('mainContent');
        content.innerHTML = '<div id="anunciosContent"></div>';
        if (window.loadAnunciosContent) {
